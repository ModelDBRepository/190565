...
T = 80; tmin = 5; tcrit = 55; nt = round(T/dt); tx = (1:1:nt)*dt;  xz = ((sin(2*tx*pi*4).*sin(2*tx*pi*6))+ 0.05*randn(1,nt))';  G = 15; Q = 1*10^4; %Scaling Weight for BPhi
...
BPhi = zeros(N,m);
z = zeros(m,1);
step = 50; %Sets the frequency of RLMS  
imin = round(tmin/dt); %Start RLMS
icrit = round((tcrit/dt)); %Stop RLMS 
...
index2 = find(v(1:100)>vpeak); 
if length(index2>0) 
    tspike(ns+1:ns+length(index2),:) = [index2,0*index2+dt*i];
end
...
%%
Z = eig(OMEGA);  %eigenvalues Prelearning 
