...
% half note.  Note that the HDTS is encoded into the supervisor here, as
% the last 64 components.  
freq = 4; 
load ode2joylong.mat;
dd = size(J); 
nnotes = dd(1);
nchord = dd(2);
...
if tr == 0 
    IPSC = IPSC*exp(-dt/td)+   JD*(length(index)>0)/(td);close
